...
Каждый `PaymentOptionDTO` повторяет authoritative `amount` и `currency` из снимка заказа, поэтому
изменение цены в каталоге после создания заказа не меняет показываемую и оплачиваемую сумму.
Коды валют задаёт host-приложение: это могут быть как фиатные валюты, так и токены приложения;
закрытого enum внутри CommerceXL нет.
